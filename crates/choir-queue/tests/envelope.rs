...
fn transfer_is_identity_at_the_corpus_it_was_measured_on() {
    ...
    for (precision, recall, prevalence) in
        [(0.43, 0.60, 1.0 / 3.0), (0.65, 0.88, 0.30), (0.47, 0.28, 0.5)]
    {
...
fn the_borba_operating_point_collapses_at_a_real_mainline_rate() {
    ...
    assert!((precision - by_counts).abs() < 1e-9, "{precision} vs {by_counts}");

    assert!(precision < 0.01, "precision at 0.4% prevalence: {precision}");
...
fn precision_falls_monotonically_as_the_population_gets_cleaner() {
    ...
    for p in [0.5, 0.33, 0.1, 0.01, 0.004, 0.001] {
        ...
        assert!(cur < last, "precision rose from {last} to {cur} at prevalence {p}");
        last = cur;
    }
}
...
fn the_required_false_positive_rate_is_what_makes_this_actionable() {
    ...
    let built = Detector { recall: 0.60, false_positive_rate: need };
...
fn differential_testing_is_bounded_by_flakiness_not_by_recall() {
    ...
    for (flake, floor, ceiling) in [
        ...
        let d = Detector { recall: generous_recall, false_positive_rate: flake };
    ...
    let flaky_high_recall = Detector { recall: 0.8, false_positive_rate: 0.01 };
    let flaky_low_recall = Detector { recall: 0.4, false_positive_rate: 0.01 };
    let clean_low_recall = Detector { recall: 0.4, false_positive_rate: 0.001 };
    assert!(
...
fn bad_inputs_are_refused_rather_than_returning_a_plausible_number() {
    ...
    assert!(Detector::from_reported(0.0, 0.6, 0.3).is_err(), "zero precision");
    assert!(Detector::from_reported(0.4, 0.0, 0.3).is_err(), "zero recall");
    assert!(Detector::from_reported(0.4, 0.6, 1.0).is_err(), "no negatives");
    assert!(required_false_positive_rate(0.004, 0.6, 1.0).is_err(), "demands zero fp");
    ...
    let dead = Detector { recall: 0.0, false_positive_rate: 0.0 };
