...
printf '{"format_version":1,"observation_id":7,"merge":"%s","report":{"verdict":"clean"},"calibration":{"target":{"met":true},"pending_interactions":0,"confidence_claim":null,"confidence_policy":null,"landing_gate_enabled":false}}\n' "$8"
"#,
    )
    .unwrap();
    #[cfg(unix)]
    ...
    std::fs::write(
        &runner,
        r#"#!/bin/sh
printf '{"format_version":1,"observation_id":8,"merge":"%s","report":{"verdict":"clean"},"calibration":{"target":{"met":true},"pending_interactions":0,"confidence_claim":null,"confidence_policy":null,"landing_gate_enabled":true}}\n' "$8"
"#,
    )
    .unwrap();
