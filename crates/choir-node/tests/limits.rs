...
fn every_served_request_is_recorded_against_the_user_who_made_it() {
    ...
    let users: Vec<&str> = lines
        ...
        .collect();
    assert_eq!(users, vec!["alice", "bob"], "lines carry the authenticated user");
...
fn the_browse_surface_is_recorded_like_every_other_route() {
    ...
    let node = served("browse-attribution", &Config { repos: &["alice/one"], ..Config::default() });

    assert_eq!(status(&["-u", "alice:a", &format!("{}/r/", node.base)]), 200);
...
fn nothing_that_could_carry_a_token_reaches_the_log() {
    ...
    status(&["-u", "alice:a", "-X", "POST", "-d", "{\"secret\":\"ALSOSECRET\"}", &submit]);

    let lines = log_lines(&node.log, 2);
    assert_eq!(lines.len(), 2, "{lines:?}");
    let text = std::fs::read_to_string(&node.log).expect("log readable");
    assert!(!text.contains("SUPERSECRET"), "a query string reached the log: {text}");
    assert!(!text.contains("ALSOSECRET"), "a request body reached the log: {text}");
    assert!(!text.to_lowercase().contains("authorization"), "a header reached the log: {text}");
    assert!(!text.contains("alice:a"), "a credential reached the log: {text}");
...
fn the_log_rotates_at_its_bound_and_keeps_one_generation() {
    ...
    assert!(rotated.exists(), "twelve requests past a 400-byte bound never rotated");
    ...
    assert!(live < 400 + 512, "the live log grew past its bound: {live} bytes");
    let mut third = node.log.into_os_string();
    third.push(".2");
    assert!(!PathBuf::from(third).exists(), "rotation kept more than two generations");
}
...
fn a_user_over_their_api_ceiling_gets_429_and_is_told_when_to_come_back() {
    ...
    let view = format!("{}/api/view", node.base);
    for i in 0..3 {
        assert_eq!(status(&["-u", "alice:a", &view]), 200, "request {i} inside the allowance");
    }
    assert_eq!(status(&["-u", "alice:a", &view]), 429, "the fourth request is refused");
    ...
    assert!((1..=20).contains(&retry), "implausible Retry-After: {retry}");
    ...
    assert!(refusals >= 2, "the refusals are in the record too: {lines:?}");
}
...
fn the_git_and_api_ceilings_are_separate_buckets() {
    ...
    let view = format!("{}/api/view", node.base);
    let refs = format!("{}/owner/repo.git/info/refs?service=git-upload-pack", node.base);

    assert_eq!(status(&["-u", "alice:a", &view]), 200);
    assert_eq!(status(&["-u", "alice:a", &view]), 429, "the API bucket is spent");
    ...
    assert_eq!(status(&["-u", "alice:a", &refs]), 429, "the git bucket never ran out");
}
...
fn the_hook_callback_is_never_limited_so_a_multi_ref_push_lands() {
    ...
    assert!(
        git(&node.work, &["clone", "-q", &format!("{url}/owner/repo.git"), dir.to_str().unwrap()])
            .status
            .success(),
        "clone failed"
    );
...
fn a_node_without_authentication_is_never_limited() {
    ...
    let view = format!("{}/api/view", node.base);
    for i in 0..8 {
        assert_eq!(status(&[&view]), 200, "request {i} was limited on an unauthenticated node");
    }
...
fn the_daemon_refuses_attribution_and_metering_without_authentication() {
    ...
    let err = daemon_refuses(&work, &["--request-log", work.join("r.jsonl").to_str().unwrap()]);
    ...
    let err = daemon_refuses(
        &work,
        &["--auth-file", auth.to_str().unwrap(), "--rate-limit-git", "0"],
    );
    assert!(err.contains("positive integer"), "{err}");

    // A flag with no value is a typo, not a default.
    let err = daemon_refuses(&work, &["--auth-file", auth.to_str().unwrap(), "--rate-limit-api"]);
