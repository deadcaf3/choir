...
| Backup and restore | Copies the log, the node fingerprint, the policy files and one git bundle per repository, and verifies the copy offline. | `./choirctl pull-backup` |
...
**Platforms.** macOS on APFS provisions workspaces with `clonefile` and installs under launchd. Linux is supported, and a btrfs volume gives snapshot-backed workspaces. Binding a non-loopback address requires TLS (`--tls-cert` and `--tls-key`).
...
On macOS, `./choirctl install` builds the binaries, mints the `~/.choir` secrets and loads a launchd job. `./choirctl url` prints the clone and push URL.
...
> [!IMPORTANT]
> Create repositories with `--create` or the installer. The `pre-receive` hook it installs is what sequences pushes into them.
