...
That warning is about ed25519 keys only. A passkey-signed entry carries its own credential key (D45), so it needs no keys file and survives a restore that keeps the log and loses everything else, which is what `scripts/flip/pull_backup.sh` does, deliberately. `choir log --verify` counts those entries on their own line, as *intact but unanchored*: the bytes are proven, and what is not proven is that the credential belonged to that account, because that binding lives in the accounts store rather than in the log. Read it as a real check that stops one step short, not as a weaker version of the ed25519 one.
