...
| `identity_state` | A key-binding precondition failed (key already bound to another operator, revoked or unbound key, channel naming a different operator) | Not a retry: a key belongs to one operator for the life of the key, and a revoked key is never rebindable. Bind a fresh key instead. `error` names which of the two applies. |
